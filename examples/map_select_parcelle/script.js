...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Prépare la couche des parcelles.
  var parcelleLayer = createParcelleLayer();
  ...
  map.on('click', function (ev) {
    ...
    // Récupère les parcelles cliquées.
    getCadastreFeatureInfo(ev.coordinate, mapView.getResolution(), mapView.getProjection())
      .then(function (json) {
        if (!json || json.type !== 'FeatureCollection') {
          throw new Error('Invalid response returned');
        }
        // Ajoute les parcelles trouvées sur la carte.
        var features = geojson.readFeatures(json);
        parcelleLayer.getSource().addFeatures(features);
      });
  });
});
